  ...
  useEffect(() => {
    if (Platform.OS === 'web') {
      const handlePopState = () => {
        // If user is authenticated and the current path would be the welcome screen
        if (isAuthenticated && window.location.pathname === '/') {
          // Redirect back to tabs
          router.replace('/(tabs)');
        }
      };
  ...
  useEffect(() => {
    if (Platform.OS === 'web' && isAuthenticated && !hasRedirectedRef.current) {
      ...
      if (window.location.pathname === '/' || window.location.pathname === '') {
        ...
        // Replace the current history entry with tabs
        router.replace('/(tabs)');
        
        // This is important: we're replacing the current history entry
        // so the user won't get stuck in a redirect loop when pressing back
      }
    }
  }, [isAuthenticated, router]);
